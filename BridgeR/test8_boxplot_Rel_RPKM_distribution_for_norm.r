setwd("C:/Users/Naoto/Documents/github/BRIC-seq_data_analysis/BridgeR/data/BridgeR_90-95_percentile_normalization")
...
input_data <- read.table("BridgeR_3_Normalized_expression_data_siStealth_siPUM1_compatible.txt",header=T)
...
png(filename='Pointplot_Comparison_of_Rel_RPKM_siStealth_vs_siPUM1_percentile_norm.png',width = 900, height = 900)
...
input_data <- read.table("BridgeR_3_Normalized_expression_data_siStealth_siCTRL_compatible.txt",header=T)
test_data <- input_data[input_data$T0_1 == 1,]
test_data <- test_data[test_data$T0_2 == 1,]
...
png(filename='Pointplot_Comparison_of_Rel_RPKM_siStealth_vs_siCTRL_percentile_norm.png',width = 900, height = 900)
