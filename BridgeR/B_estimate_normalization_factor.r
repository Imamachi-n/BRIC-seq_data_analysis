...
###Prepare_file_infor###
hour <- c(0,1,2,4,8,12)
test <- c("A","B","C","D","E","F") ##Should be changed!!
time_point <- length(hour)
unit <- "hr"
filename <- 'stable_genes_distribution.png'
...
log_path <- paste('C:/Users/Naoto/Documents/github/BRIC-seq_data_analysis/BridgeR/data/genes_RefSeq_result_mRNA_stable_genes.fpkm_table',sep="")
output_path <- paste('C:/Users/Naoto/Documents/github/BRIC-seq_data_analysis/BridgeR/data/genes_RefSeq_result_mRNA_normalization_factor.fpkm_table',sep="")

###Load_input_file###
input_file <- fread(input_path, header=T)
label <- c("gr_id","symbol","accession_id","locus")
for (x in 1:time_point){
    label <- append(label, test[x]) ##Should be changed!!
}
setnames(input_file,label) #Relabel header

###print_header###
cat("gr_id","symbol","accession_id","locus", sep="\t", file=log_path)
cat("\t", file=log_path, append=T)
hour_label <- NULL
for(x in hour){
    hour_label <- append(hour_label, paste(x, unit, sep=""))
}
cat(hour_label, sep="\t", file=log_path, append=T)
cat("\n", sep="", file=log_path, append=T)
